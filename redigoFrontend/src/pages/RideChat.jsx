...
const RideChat = () => {
  ...
  try {
    ...
    if (rideData.success && rideData.ride) {
      ...
      // Extract driver and passenger info safely
      ...
      // ✅ Set the "other user" based on that
      ...
      if (targetUser) {
        ...
        // Fallback if ride doesn’t have passenger yet
        setOtherUser({
          _id: "unknown",
  ...
  const fetchPassengerInfo = async () => {
    try {
      ...
      const chatResponse = await fetch(`http://localhost:5000/api/chat/passenger/${rideId}`, {
        headers: { Authorization: token },
      });
      ...
      // Fallback: Set placeholder that will be updated when message arrives
      setOtherUser({
        _id: "passenger",
        name: "Passenger",
        role: "Passenger",
  ...
  useEffect(() => {
    if (currentUser && rideId && rideDetails) {
      ...
      const handleReceiveMessage = (data) => {
        ...
        // ✅ Update passenger info if driver receives first message
...
const fetchChatHistory = async () => {
  ...
  try {
    ...
    const res = await fetch(
      `http://localhost:5000/api/chat/${rideId}/${currentUser._id}`,
      { headers: { Authorization: token } }
    );
  ...
  // ✅ Identify the "other user" (the one you are chatting with) based on message data
// ✅ Get passenger name from messages after chat history loads
useEffect(() => {
  if (messages.length > 0 && currentUser?._id && otherUser?.name === "Passenger") {
    // Find first message from someone who is NOT the current user
...
}, [messages, currentUser]); // ✅ Removed otherUser from dependencies
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto pt-20 pb-8">
        {/* ✅ 1-on-1 Chat Header - Shows only the other person */}
