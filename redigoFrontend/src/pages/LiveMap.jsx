...
// ✅ Create socket outside component to prevent re-creation
// ✅ FIXED: Updated socket initialization
let socket = null;
...
const initSocket = () => {
  if (!socket || socket.disconnected) {
    ...
    socket = io(socketUrl, {
      // ✅ CRITICAL: Specify the correct path
      path: '/socket.io/',
...
const LiveMap = () => {
  ...
  // ✅ Marker refs to prevent re-creation
  ...
  // ✅ Load Google Maps API (moved outside component logic)
  ...
  // ✅ Initialize component with ride data (run only once)
  useEffect(() => {
    let isMounted = true;
      ...
      try {
        ...
        if (rideId) {
          ...
          try {
            ...
            // ✅ Fix: Check driver info correctly based on API structure
            ...
            if (isDriver) {
              // ✅ Fix: Access passenger info correctly (if exists)
              setOtherUserInfo({
              ...
              // ✅ Fix: Access driver info correctly from API structure
              setOtherUserInfo({
  ...
  // ✅ Setup geolocation (run only when needed)
  useEffect(() => {
  ...
  // ✅ Calculate ride route when map loads (run only once)
  useEffect(() => {
