...
// import { Delete, Edit2, MapPin, Calendar, Clock, Users, IndianRupee, Car, BookOpen, TrendingUp, Wallet } from "lucide-react";
...
//     try {
//       const token = localStorage.getItem("token");
//       const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/rides/user-rides`, {
//         headers: {
//           Authorization: `${token}`,
//         },
//       });
...
//     try {
//       const token = localStorage.getItem("token");
//       const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/rides/booked-rides`, {
//         headers: {
//           Authorization: `${token}`,
//         },
//       });
...
//       console.log('✅ Booked rides loaded:', res.data.bookedRides?.length || 0);
//     } catch (err) {
...
//     try {
//       const token = localStorage.getItem("token");
//       const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/user/dashboard-stats`, {
//         headers: {
//           Authorization: `${token}`,
//         },
//       });
...
//   // Calculate stats from available data if API endpoint doesn't exist
...
//       // Calculate savings from booked rides
//       const totalSavings = bookedRides.reduce((sum, ride) => {
//         return sum + (ride.price || 0);
//       }, 0);

//       // Count completed, active, and cancelled rides
...
//       // Count total passengers across all rides
...
//   // Recalculate stats when rides data changes
//   useEffect(() => {
...
//     navigate(`/edit-ride/${ride._id}`, { state: { ride } });
//   };

//   // Cancel booking handler
//   const handleCancelBooking = async (rideId, bookingId) => {
//     if (!window.confirm("Are you sure you want to cancel this booking?")) return;

//     try {
//       const token = localStorage.getItem("token");
//       await axios.delete(`${import.meta.env.VITE_BACKEND_URL}/api/rides/${rideId}/cancel-booking`, {
//         headers: {
//           Authorization: `${token}`,
//         },
//       });
...
//       setBookedRides(prev => prev.filter(ride => ride._id !== rideId));
//       alert("Booking cancelled successfully!");
//       console.log('✅ Booking cancelled:', rideId);
//     } catch (err) {
...
function DashboardPage() {
  ...
  const fetchUserRides = async () => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/api/rides/user-rides`,
        { headers: getAuthHeader() }
      );
      setUserRides(res.data.rides || []);
      console.log("✅ User rides loaded:", res.data.rides?.length || 0);
  ...
  const fetchBookedRides = async () => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/api/rides/booked-rides`,
        { headers: getAuthHeader() }
      );
      setBookedRides(res.data.bookedRides || []);
      console.log("✅ Booked rides loaded:", res.data.bookedRides?.length || 0);
  ...
  // ✅ Fetch dashboard statistics (money spent, earned, etc.)
  const fetchDashboardStats = async () => {
    try {
      const res = await axios.get(
